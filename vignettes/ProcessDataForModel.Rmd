...
## Data analysis to be performed before using NFDS model
...
3. Construct a pangenome:
    a. ggCaller 1.3.4 [@horsfieldAccurateFastGraphbased2023a] can be used for gene calling using the SPARC dataset for annotation [@croucher_population_2015], [@croucher_data_2016] and with the options `--gene-finding-only --annotation sensitive`
    b. Bakta [@schwengers_bakta_2021] with default parameters can be used for functional annotation (if wanted)
    c. Panaroo v1.5.2 [@tonkin-hill_producing_2020] can be run with the options `--refind-mode off --clean-mode moderate --family_threshold 0.7 --min_trailing_support 2 --trailing_recursive 0 --min_edge_support_sv 2 --edge_support_threshold 0.0 --aligner none`

## Read in data

The metadata file should include Lane_id, collection year, serotype, vaccine type, and GPSC. You will likely want to read in as follows:

```{r, eval=FALSE}
MetaDataWithGPSC <- read.csv("MetaDataWithGPSC.csv")
```

Here we read in example data that comes with the package using `data()`:
...
The column names of your own `data.frame` must match this example, i.e.:

```{r}
colnames(MetaDataWithGPSC)
```

The pangenome matrix must have the Lane_ids as column names. When using Panaroo, this should be the default output format. You will likely want to read in as follows:

```{r, eval=FALSE}
gene_presence_absence_mtx <- read.csv("gene_presence_absence.csv", header=TRUE, check.names=FALSE)
```

Again, we read in example data here:
...
### Convert the pangenome matrix that we read in earlier (`gene_presence_absence_mtx`) to boolean form
...
### Create time point-specific presence absence matrices for the first and last time points
...
## Simulate the model with your input data
...
The simulated data is a `data.frame`: the columns represent time steps of the model simulation, and the rows represent GPSCs.

```{r}
head(simulated_data)
```

# Alternatively, you can also simulate serotype frequencies with the model

```{r}
simulated_data_serotypes <- simulate_model_serotypes(model_parameters = model_params, simulation_steps = time_steps)
```

The simulated data is a `data.frame`: the columns represent time steps of the model simulation, and the rows represent serotypes.

```{r}
head(simulated_data_serotypes)
```

For fitting the model, you will also need to calculate frequencies of GPSCs that you want to fit to - see below.

### Calculate GPSC counts for each data time point

As mentioned above, this is needed for fitting the model to data.
...
## Example of fitting the model to data

Note: fitting the model to data will take longer to run than just simulating the model.
